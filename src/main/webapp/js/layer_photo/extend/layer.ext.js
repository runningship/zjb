...
layer.photos = function(options){
    ...
    if(json){
        ...
        if(json.status === 1){
            ...
            if(data.length > 0){
                ...
                log.zanCount = data[json.start].zanCount;
                log.shitCount = data[json.start].shitCount;
        ...
        page: {
            html: '<div class="xubox_bigimg"><img src="'+ log.thissrc +'" alt="'+ (log.name || '') +'" layer-pid="'+ (log.pid || '') +'"><div class="xubox_imgsee">'+ function(){
                if(log.imgLen > 1){
                    ...
                    +'<span class="xubox_zanbox">'
                    +'<a href="" class="xubox_zan xubox_zanZan"><i class="iconfont">&#xe620;</i><em></em></a>'
                    +'<a href="" class="xubox_zan xubox_zanShit"><i class="iconfont">&#xe624;</i><em></em></a>'
                    +'</span>'
                    +'<span class="xubox_imgtit"><a href="javascript:;">'+ log.imgsname +' </a><em>'+ log.imgIndex +'/'+ log.imgLen +'</em></span></div></div></div>'+ function(){
                if(options.html){
            ...
            log.imgzan = log.imgbar.find('.xubox_zanbox');
            log.layero = layero;
    ...
    log.event = function(){
        ...
        conf.zanZan = function(){
            var zanJsonData=json.data[log.imgIndex-1];
            YW.ajax({
                type: 'POST',
                url: '/c/mobile/houseImage/zanHouseImage',
                data:{hiid:zanJsonData.hiid , zan:zanJsonData.zan^1},
                dataType:'json',
                mysuccess: function(data){
                	if(data.result==0){
                        if(zanJsonData.zan>0){
                		    zanJsonData.zanCount--;
                            zanJsonData.zan=0;
                            log.imgzan.find('.xubox_zanZan em').text(zanJsonData.zanCount);
        ...
        conf.zanShit = function(){
            ...
            YW.ajax({
                ...
                data:{hiid:zanJsonData.hiid , shit:zanJsonData.shit^1},
                dataType:'json',
                mysuccess: function(data){
                	if(data.result==0){
                        if(zanJsonData.shit>0){
                            zanJsonData.shitCount--;
                            zanJsonData.shit=0;
                            log.imgzan.find('.xubox_zanShit em').text(zanJsonData.shitCount);
