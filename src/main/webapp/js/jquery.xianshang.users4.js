...
function fun_get_comp(a){
    ...
    if($(".get_comp"+as).length>0){
        ...
        getComp.empty();
        getComp.prepend('<option value="">请选择区域</option>');
        ...
        if(getUserTreeStr.result.length=1){
            getComp.find("option").eq(1).attr("selected",true);
            getComp.attr('disabled',true);
        }
        getComp.change();
    }else{fun_get_quyu(a);}
}
    ...
    if($(".get_quyu"+as).length>0){
        ...
        if($(".get_comp"+as).length>0){
            ...
            if(getUserTreeStr.result[comp_index]){
                ...
                if(getUserTreeStr.result[comp_index].length=1){
                    getQuyu.find("option").eq(1).attr("selected",true);
                    getQuyu.attr('disabled',true);
                }
            }
